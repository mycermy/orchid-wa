...
class Devices_EditScreen extends Screen
{
    ...
    {
        return $this->device->exists ? 'Edit Device' : 'Add New Device';
    }
    ...
    public function commandBar(): iterable
    {
        return [
            ...
            Link::make(__('Cancel'))
                ->icon('bs.x-circle')
                // ->canSee($this->device->exists == false)
    ...
    {
        return [
            Layout::block(Devices_FieldLayout::class)
                ->title('WhatsApp Device Information')
                ->commands(
                    Button::make(__('Save'))
                        ->type(Color::BASIC)
                        ->icon('bs.check-circle')
            ...
            Layout::block(Layout::view('partials.scanqr'))
            ->title('To use WhatsApp on your computer:')
            ->description('
            ...
            ')
            ->canSee($this->device->exists && $this->device->status != 'AUTHENTICATED'),
    ...
    {
        $device->fill($request->get('device'));

        $device->fill(['status' => 'New']);

        $device->save();

        Toast::info(__('Device details was saved.'));

        return redirect()->route('platform.whatsapp.devices');
    }

    public function unlinkDevice(Device $device)
    {
        $response = Http::delete(env('URL_WA_SERVER').'/sessions/'.$device->name);

        $device->update(['status' => 'UNLINKED']);

        Toast::info(__('Device unlinked.'));
